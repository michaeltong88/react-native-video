...
enum class EventTypes(val eventName: String) {
    ...
    EVENT_SHOW_CONTROLS("onShowControls"),
    EVENT_HIDE_CONTROLS("onHideControls"),
    EVENT_RESUME("onResumePlayback"),
    EVENT_PAUSE("onPausePlayback");
...
class VideoEventEmitter {
    ...
    lateinit var onShowControls: () -> Unit
    lateinit var onHideControls: () -> Unit
    lateinit var onResumePlayback: () -> Unit
    lateinit var onPausePlayback: () -> Unit
    ...
    fun addEventEmitters(reactContext: ThemedReactContext, view: ReactExoplayerView) {
        ...
        if (dispatcher != null) {
            ...
            onShowControls = {
                event.dispatch(EventTypes.EVENT_SHOW_CONTROLS)
            }
            onHideControls = {
                event.dispatch(EventTypes.EVENT_HIDE_CONTROLS)
            }
            onResumePlayback = {
                event.dispatch(EventTypes.EVENT_RESUME)
            }
            onPausePlayback = {
                event.dispatch(EventTypes.EVENT_PAUSE)
            }
        }
    }
